...
INSERT INTO public.invoice_templates (user_id, name, description, layout_type, color_scheme, is_default)
SELECT 
  u.id,
  'Template Moderne',
  'Template moderne avec design épuré',
  'modern',
  '{"primary": "#3b82f6", "secondary": "#64748b", "accent": "#06b6d4"}',
  true
FROM auth.users u
WHERE NOT EXISTS (
  SELECT 1 FROM public.invoice_templates t WHERE t.user_id = u.id
);

INSERT INTO public.invoice_templates (user_id, name, description, layout_type, color_scheme)
SELECT 
  u.id,
  'Template Classique',
  'Template traditionnel pour un look professionnel',
  'classic',
...
SELECT 
  u.id,
  'Template Coloré',
  'Template vibrant avec couleurs dynamiques',
  'colorful',
